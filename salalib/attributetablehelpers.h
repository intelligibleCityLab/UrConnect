...
namespace dXreimpl{
	...
	inline PafColor getDisplayColor(const AttributeKey& key, const AttributeRow& row, const AttributeTableView& tableView, PafColor color2,bool checkSelectionStatus = false)
	{
	...
	inline PafColor getDisplayColor(const AttributeKey& key, const AttributeRow& row, const AttributeTableView& tableView, PafColor color2, std::vector<int> &Ref_number_list, bool checkSelectionStatus = false)
	{
		std::vector<int>::iterator it;
		it = find(Ref_number_list.begin(), Ref_number_list.end(), key.value);
		if (it != Ref_number_list.end()) {
			return color2;
		}
