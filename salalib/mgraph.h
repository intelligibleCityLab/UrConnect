...
public:
   ...
   bool setCurSel(QtRegion& r, bool add = false)  // set current selection
   {
	   ...
	   else if (m_view_class & VIEWDATA)
	   {
		   ...
		   if (add == true)
		   {
			   auto mfefe = getDisplayedDataMap().setCurSel_my_define(r, add, Ref_number_list);
			   sort(Ref_number_list.begin(), Ref_number_list.end());
			   Ref_number_list.erase(unique(Ref_number_list.begin(), Ref_number_list.end()), Ref_number_list.end());
			   return mfefe;
		   }
		   else
		   {
		...
		//QString qstrAngleThreshold = pDisplaydow->ui->lineEdit_22->text();
	   }
