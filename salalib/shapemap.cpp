...
void ShapeMap::removeIds(std::vector<int> &ids)
{
...
bool ShapeMap::setCurSel_my_define(QtRegion &r, bool add, std::vector<int> &Ref_number_list)
{
    ...
    if (r.bottom_left == r.top_right) {
        ...
        if (index == -1) {
			...
			Ref_number_list.push_back(index);
        ...
        for (int i = bl.x; i <= tr.x; i++) {
            for (int j = bl.y; j <= tr.y; j++) {
                const std::vector<ShapeRef> &shapeRefs =
					...
					if (shapeRefs.size() > 0)
				{
					for (int ii = 0; ii < shapeRefs.size(); ii++)
						Ref_number_list.push_back(shapeRefs[ii].m_shape_ref);
				}
                for (const ShapeRef &shape : shapeRefs) {
...
bool ShapeMap::output(std::ofstream &stream, char delimiter) {
    stream << "Ref";
    //������������������ m_map_type ������������������������������������������������������������������������������������������������������������������������������������������������ m_map_type ��������������������������� LINEMAP������������������������������������ "cx" ��������� "cy"��������������������������������������������� LINEMAP������������������������������������ "x1"���������"y1"���������"x2" ��������� "y2"
    ...
    for (int i = bl.x; i <= tr.x; i++) {
        for (int j = bl.y; j <= tr.y; j++) {
            ...
            for (const ShapeRef &shape : shapeRefs) {
                ...
                //if (x != shape.m_shape_ref) {
                //    std::string s = "x!=shape.m_shape_ref\n";
                //    OutputDebugStringA(s.c_str());
                //}
                int x = shape.m_shape_ref;
...
std::vector<std::pair<SimpleLine, PafColor>> ShapeMap::getAllLinesWithColour(PafColor color) {
	...
	for (auto &refShape : allShapes) {
		...
		//if (m_attributes->getRow(AttributeKey(refShape.first)).isSelected())
			//Ref_number_list.push_back(refShape.first);
...
std::vector<std::pair<SimpleLine, PafColor>> ShapeMap::getAllLinesWithColour(PafColor color, std::vector<int> &Ref_number_list)
{
	std::vector<std::pair<SimpleLine, PafColor>> colouredLines;
	std::map<int, SalaShape> &allShapes = getAllShapes();
	int k = -1;
...
std::vector<std::pair<SimpleLine, PafColor>> ShapeMap::getAllLinesWithColour() {
    ...
    for (auto &refShape : allShapes) {
		...
		int key = AttributeKey(refShape.first).value;
		AttributeRow &row = m_attributes->getRow(AttributeKey(refShape.first));

		if (!row.isSelected() && FixedColoredLinesMap.count(key))
			colour = FixedColoredLinesMap[key];
		else {
			colour = dXreimpl::getDisplayColor(AttributeKey(refShape.first),
				row,
