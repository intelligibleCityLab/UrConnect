...
bool VGAVisualLocal::run(Communicator *comm, PointMap &map, bool simple_version) {
    ...
    if (!simple_version) {
        cluster_col = static_cast<int>(map.getAttributeTable().insertOrResetColumn("Visual Clustering Coefficient"));
        control_col = static_cast<int>(map.getAttributeTable().insertOrResetColumn("Visual Control"));
        controllability_col = static_cast<int>(map.getAttributeTable().insertOrResetColumn("Visual Controllability"));
    }
