...
void ShapeObject::Assign(const SHPObject* obj)
{
	if (obj != 0) {
		...
		if (vertexCount > 0) {
			...
			sPoint<double> *pptr = vertices;
			sPoint<double> *pend = pptr + vertexCount;
			while (pptr < pend) {
				pptr->Set(*xptr, *yptr);
				xptr++;
				yptr++;
...
const sPoint<double>* ShapeObject::GetVertices() const
{
	return const_cast<const sPoint<double>*>(vertices);
}
