...
public:
	...
	//add
	void addLines(std::vector<std::vector<double>> &coordinateData);
	void removeLines(std::vector<int> &id_list);
...
public:
   ...
   void RenameColumnByCol(int col, std::string newName);
   ...
   void OnToolsTopomet();
	...
	void OnFileImport(QString filename);
    void OnFileImport();
	void OnTXTFileImport(std::string txtfilenameStr);
	void OnStreamImport(std::string file_path);
	void OnFileExport();
