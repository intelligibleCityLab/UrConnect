...
QGraphDoc::QGraphDoc(const QString &author, const QString &organisation)
{
   ...
   m_meta_graph->setProperties(std::string((const char *)author.toLocal8Bit()), std::string((const char *)organisation.toLocal8Bit()), date, std::string((const char *)version.toLocal8Bit()));
...
void QGraphDoc::UpdateLinesID()
{
	((MainWindow *)m_mainFrame)->UpdateLinesID();
}
...
void QGraphDoc::OnTXTFileImport(std::string txtfilenameStr)
{
	//������������������������������������������������������������������������������������������������������������������������������������������������������������������������������������������������������������������
	bool graphHadNullBoundsBeforeImport = m_meta_graph->getBoundingBox().atZero();

	QString txtfilepath = QString(QString::fromLocal8Bit(txtfilenameStr.c_str()));
	QFilePath filepath(txtfilepath);
	QString ext = filepath.m_ext;

	//std::string str = std::string((const char *)(filepath.m_name).toLocal8Bit());

	// OS files don't have GML suffix for some bizarre reason ������������������������������������������������OS������������������������GML������������
	if (ext == tr("")) {
		ext = tr("GML");
	}
	int graph_option = MetaGraph::ADD;

	std::ifstream file(txtfilepath.toLatin1());
	if (file.fail())
	{
		QMessageBox::warning(this, tr("Warning"), tr("Unable to read text file.\nPlease check that another program is not using it."),
			QMessageBox::Ok, QMessageBox::Ok);
	...
	else
	{
		...
		bool mapParsed = depthmapX::importFile(*m_meta_graph,
			...
			std::string((const char *)(filepath.m_name).toLocal8Bit()),
...
void QGraphDoc::OnStreamImport(std::string file_path) {
	extern std::istream inFileToMap;

	//������������������
	extern std::stringbuf buf;
	std::stringbuf buf2(buf.str());
	std::istream inFileToMap2(&buf2);
	std::ostream outFileToMap2(&buf2);

	//������������������������������������������������������������������������������������������������������������������������������������������������������������������������������������������������������������������
	bool graphHadNullBoundsBeforeImport = m_meta_graph->getBoundingBox().atZero();
	QString txtfilepath = QString(QString::fromLocal8Bit(file_path.c_str()));
	QFilePath filepath(txtfilepath);
	QString ext = filepath.m_ext;
...
void QGraphDoc::OnFileImport(QString filename) {
	if (m_communicator) {
		return; // Locked
	}
	...
	if (ext == tr("CAT") || ext == tr("DXF") || ext == tr("NTF") || ext == tr("RT1") || ext == tr("MIF") || ext == tr("GML") || ext == tr("")) {
		...
		if (ok) {
			...
			if (ext != tr("RT1") && ext != tr("NTF") && ext != tr("GML")) {  // ntf, gml & rt1 use filesets (all others use standard file at a time) ntf������gml������rt1������������������������������������������������������������������������������������������������������������������������������
				m_communicator->SetInfile(qPrintable(filename));
			}
			if (ext != tr("MIF")) {
				...
				int thedot = filename.lastIndexOf('.');
				QString infile2 = filename.left(thedot + 1) + tr("mid");
...
void QGraphDoc::OnFileExport() 
{
   ...
   QString defaultname = data_map_name;
   ...
   std::string outfile_name = std::string((const char *)outfile.toLocal8Bit());
   ...
   if (ext != tr("MIF") && ext != tr("GRAPH") && ext != tr("NET")) 
   {
       ...
       stream.close();
        ...
        if (m_meta_graph->write(outfile_name, METAGRAPH_VERSION, true) != MetaGraph::OK) { // <- true writes current layer only
	        QMessageBox::warning(this, tr("Notice"), tr("Sorry, unable to open file for export"), QMessageBox::Ok, QMessageBox::Ok);
        }
    }
    else if (ext == tr("NET")) {
...
void QGraphDoc::OnTranferToSegment() {
	if (m_communicator) {
		//QMessageBox::warning(this, tr("Notice"), tr("Sorry, cannot export as another process is running"), QMessageBox::Ok, QMessageBox::Ok);
		return;  // Locked
	}
	m_communicator = new CMSCommunicator();
	extern QString data_map_name;
	m_communicator->SetString(data_map_name);
	m_communicator->SetOption(true ? 1 : 0, 0);  // <- option 0 used for retain original flag
	m_communicator->SetOption(true ? 1 : 0, 1);   // <- option 1 used for push values flag
...
void QGraphDoc::OnLayerConvert() 
{
   ...
   extern QString data_map_name;
   m_communicator->SetString(data_map_name);
   m_communicator->SetOption(1, 0);  // <- option 0 used for retain original flag
   m_communicator->SetOption(1, 1);   // <- option 1 used for push values flag
...
bool QGraphDoc::OnNewDocument()
{
	...
	extern QString mapViewName;
	m_base_title = mapViewName;
...
void QGraphDoc::OnAddColumn() 
{
   ...
   while (dlg.exec() == QDialog::Accepted && !success) {
	  ...
	  else if (dlg.m_object_name.size() > 10) {
		  QMessageBox::warning(this, tr("Warning"), tr("Sorry, the number of characters in the column name must be less than or equal to 10"), QMessageBox::Ok, QMessageBox::Ok);
		  return;
	  }
      else {
...
void QGraphDoc::OnRenameColumn() 
{
   ...
   std::string fieldname = tab->getColumnName(col);
   // -1 is reference number, -2 is displaying nothing (-2 shouldn't happen but is)
   if (col == -1 || col == -2 || m_meta_graph->isAttributeLocked(col) || fieldname=="ID") {
...
void QGraphDoc::RenameColumnByCol(int col, std::string newName)
{
	AttributeTable *tab = &(m_meta_graph->getAttributeTable());
	if (tab->getNumColumns() <= col)
		return;
	tab->renameColumn(tab->getColumnName(col), newName);
}
...
int QGraphDoc::RenameColumn(AttributeTable *tab, int col)
{
   ...
   while (dlg.exec() == QDialog::Accepted && !success && dlg.m_object_name != colname) {
       ...
       std::string newColName = std::string((const char*)dlg.m_object_name.toLocal8Bit());
      if (tab->hasColumn(newColName)) {
		  QMessageBox::warning(this, tr("Notice"), tr("Sorry, another column already has this name, please choose a unique column name"), QMessageBox::Ok, QMessageBox::Ok);
      }
	  else if (newColName.size() > 10) {
		  QMessageBox::warning(this, tr("Notice"), tr("Sorry, the number of characters in the column name must be less than or equal to 10"), QMessageBox::Ok, QMessageBox::Ok);
	  }
	  else {
...
void QGraphDoc::OnRemoveColumn() 
{
  // just check you really are viewing the layers:
   ...
   else if (QMessageBox::Yes == QMessageBox::question(this, tr(""), tr("Are you sure you want to delete the currently displayed column?"), QMessageBox::Yes|QMessageBox::No, QMessageBox::No)) {
...
void QGraphDoc::addLines(std::vector<std::vector<double>> &coordinateData)
{
	for (int i = 0; i<int(coordinateData.size()); i++)
	{
		Point2f m_tempFirstPoint(coordinateData[i][0], coordinateData[i][1]);
		Point2f worldPoint(coordinateData[i][2], coordinateData[i][3]);

		if (m_meta_graph->makeShape(Line(m_tempFirstPoint, worldPoint))) {
			modifiedFlag = true;
			SetRedrawFlag(QGraphDoc::VIEW_ALL, QGraphDoc::REDRAW_GRAPH, QGraphDoc::NEW_DATA);
		}
	}
...
void QGraphDoc::removeLines(std::vector<int> &id_list)
{
	m_meta_graph->getDisplayedDataMap().removeIds(id_list);
}
