...
void RenderThread::SetRunOver(bool flag) {
	mutex_run.lock();
	this->run_over = flag;
	mutex_run.unlock();
}

bool RenderThread::IsRunOver() {
	mutex_run.lock();
	bool flag = this->run_over;
	mutex_run.unlock();
	return flag;
}
...
void RenderThread::run()
{
   ...
   if (comm) {
      ...
      case CMSCommunicator::MAKEUSERSEGMAPSHAPE:
		 name = std::string((const char *)(comm->GetString()).toLocal8Bit());
         ok = pDoc->m_meta_graph->convertDataToSegment( comm, name, (comm->GetOption(0) == 1), (comm->GetOption(1) == 1) );
         if (ok) {
            if (comm->GetOption(0) == 0) {
               // note: there is both a new table and a deleted table, but deleted table leads to a greater redraw:
               pDoc->SetUpdateFlag(QGraphDoc::DELETED_TABLE);
