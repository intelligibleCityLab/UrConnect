...
void QDepthmapView::updateBackgroundColor(QColor newColor) {
...
{
   short zDelta = static_cast<short>(e->angleDelta().y());
   ...
   Point2f newCentre = ViewHelpers::calculateCenter(position.toPoint(), centre, zoomFactor);
   ...
   if (state & MetaGraph::POINTMAPS && pDoc->m_meta_graph->getViewClass() & MetaGraph::VIEWVGA) {
      ...
      while ( map.findNextPoint() ) {
         ...
         if (color.alphab() != 0) { // alpha == 0 is transparent
            ...
            stream << "<use fill=\"" << SVGColor(color) << "\" x=\"" << p.x() - spacer << "\" y=\"" << p.y() - spacer << "\" xlink:href=\"#a\" />\n";
         }
      }
      stream << "</g>\n";
   }
      ...
      if (first || color != oldcolor || closed != oldclosed) {
         ...
         if (closed) {
            if (showlines) {
               ...
               stream << "<g fill=\"" << SVGColor(color) << "\" stroke=\"none\">\n";
            }
         }
         else {
            stream << "<g fill=\"none\" stroke=\"" << SVGColor(color) << "\">\n";
         }
         oldcolor = color;
         oldclosed = closed;
      }
