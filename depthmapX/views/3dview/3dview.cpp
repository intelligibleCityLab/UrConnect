...
void Q3DView::ReloadLineData()
{
   ...
   if (pDoc->m_meta_graph && pDoc->m_meta_graph->getState() & MetaGraph::LINEDATA) {
      ...
      m_pointcount = static_cast<int>(lines.size() * 2);
      if (m_pointcount) {
   ...
   if (finite(wz1) && finite(wz2) && wz1 > 0 && wz2 < 0) {
      ...
      if (pDoc->m_meta_graph && pDoc->m_meta_graph->viewingProcessedPoints()) {
         ...
         if (pointmap.getPoint(pix).filled()) {
            ...
            m_mannequins.push_back( QMannequin(p2, static_cast<int>(m_agents.size()-1)) );
      ...
      while (file) {
         ...
         if (elementname == "trace") {
            ...
            if (m_traces.back().events.size() >= 1) {
               ...
               p.normalScale(m_region);
               m_mannequins.push_back( QMannequin(p,static_cast<int>(m_traces.size()-1),true) );
...
{
    OnMouseWheel(0, event->angleDelta().y(), event->position().toPoint());
}
