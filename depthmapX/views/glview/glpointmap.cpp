...
void GLPointMap::loadGLObjectsRequiringGLContext(const PointMap& currentPointMap) {
    QImage data(static_cast<int>(currentPointMap.getCols()),static_cast<int>(currentPointMap.getRows()), QImage::Format_RGBA8888);
    data.fill(Qt::transparent);

    for (int y = 0; y < static_cast<int>(currentPointMap.getRows()); y++) {
        for (int x = 0; x < static_cast<int>(currentPointMap.getCols()); x++) {
            PixelRef pix(x, y);
